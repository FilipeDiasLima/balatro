...
export const fragmentShaderCode2 = ({
...
float mid_flash = 0.01;
float vort_offset = 0.;

#define PIXEL_SIZE_FAC 700.
#define GREEN vec4(45./255.,95./255., 74./255., 1)
...
vec4 effect(vec3 screen_coords, float scale) {
    ...
    if (smoke_res < 0.4) {
        smoke_res = (smoke_res - 0.4)*0.2 + 0.2;
    }

    float c1p = max(0.,1. - 0.8*abs(1.-smoke_res));
    float c2p = max(0.,1. - 0.8*(smoke_res));
    float cb = 1. - min(1., c1p + c2p);

    vec4 ret_col = colour_1*c1p + colour_2*c2p + vec4(cb*GREEN.rgb, cb*colour_1.a);
    float mod_flash = max(mid_flash*0.4, max(c1p, c2p)*5. - 4.4) + mid_flash*max(c1p, c2p);
